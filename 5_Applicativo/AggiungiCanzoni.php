...
if ($metodo === 'POST' && isset($_POST['invia'])) {
    ...
    //Query che servirà per controllare se la canzone è già presente nel DB principale delle canzoni
    $query_sql_verifica = "select * from canzone where Titolo = '$titolo' and Autore = '$artista' and Album = '$album'";
    $risultato2 = $conn->query($query_sql_verifica);

    //Controllo per vedere se la canzone è già esistente, se non c'è viene aggiunta
    if($risultato2->num_rows == 0){
        ...
        //Si assiccura che tutti i dati che sono stati ricevuti siano delle stringhe
            ...
            if($conn->query($query_sql)){
                ...
                echo "<script> setTimeout(function() { window.location.href = 'playlist.php'; }, 100); </script>";
            } else {
    ...
    //Prendo l'id della canzone tramite questa query
    $controllo = "SELECT id
                      FROM canzone
                      where Titolo = '$titolo'
                      and Autore = '$artista'
